// Seeded full-game benchmark for the vendored EGTB engine, driven through
// the real adapter (src/ai-worker.js) in Node.
// Usage: node scripts/benchmark-egtb.mjs [--games=N] [--max-moves=N]
...
// Game rules run in the production GameCore.
...
// The adapter expects worker globals.
const replies = [];
...
async function chooseMove(board) {
    const requestId = ++requestSequence;
    const before = replies.length;
    self.onmessage({ data: { type: "choose-move", requestId, generation: 1, board } });
    const deadline = Date.now() + 60000;
    while (replies.length === before && Date.now() < deadline) {
        await new Promise((resolveNext) => setTimeout(resolveNext, 5));
    }
    const reply = replies[replies.length - 1];
    if (replies.length === before || reply.requestId !== requestId) {
        throw new Error("engine did not answer");
    }
...
for (let seed = 1; seed <= games; seed++) {
    ...
    while (Core.hasPossibleMoves(board) && moves < maxMoves) {
        ...
        const direction = await chooseMove(board.map((row) => [...row]));
